...
public class JenkinsJobManagement extends AbstractJobManagement {
  ...
  public JenkinsJobManagement(PrintStream outputLogger, Map<String, ?> envVars, File workspace) {
    this(outputLogger, envVars, null, new FilePath(workspace.getAbsoluteFile()),
        ...
        if (seedJobName != null) {
          ...
          if (seedReference != null) {
            ...
            if (!seedJobName.equals(existingSeedJobName) &&
              ...
              throw new DslException(format(Messages.CreateOrUpdateConfigFile_SeedCollision(), path,
                  existingSeedJobName));
            }
          }
        }
  ...
  public void createOrUpdateView(String path, String config, boolean ignoreExisting) {
    ...
    try {
      ...
      if (parent instanceof ViewGroup) {
        ...
        if (view == null) {
          if (parent instanceof ModifiableViewGroup) {
            ...
            throw new DslException(
                format(Messages.CreateView_UnsupportedParent(), parent.getFullName(),
                    parent.getClass()));
          }
        } else if (!ignoreExisting) {
          inputStream.reset();
        ...
        throw new DslException(format(Messages.CreateView_UnsupportedParent(), parent.getFullName(),
            parent.getClass()));
      }
    } catch (IOException e) {
  ...
  public void createOrUpdateUserContent(UserContent userContent, boolean ignoreExisting) {
    ...
    try {
      ...
      throw new DslException(
          format(Messages.CreateOrUpdateUserContent_Exception(), userContent.getPath(),
              e.getMessage())
      );
    }
  }
  ...
  @Override
  public InputStream streamFileInWorkspace(String relLocation)
  ...
  @Override
  public String readFileInWorkspace(String jobName, String relLocation)
      throws IOException, InterruptedException {
    ...
    if (item instanceof AbstractProject) {
      ...
      if (workspace != null) {
        try {
          ...
          logWarning(
              format(Messages.ReadFileFromWorkspace_JobFileNotFound(), relLocation, jobName));
        }
      } else {
        logWarning(
            format(Messages.ReadFileFromWorkspace_WorkspaceNotFound(), relLocation, jobName));
      }
    } else {
  ...
  public void logPluginDeprecationWarning(String pluginShortName, String minimumVersion) {
    ...
    if (plugin != null && plugin.getWrapper().getVersionNumber()
        .isOlderThan(new VersionNumber(minimumVersion))) {
      logDeprecationWarning(
          "support for " + plugin.getWrapper().getDisplayName() + " versions older than "
              + minimumVersion
      );
    }
  }
  ...
  public void requireMinimumPluginVersion(String pluginShortName, String version,
    ...
    if (plugin == null) {
      failOrMarkBuildAsUnstable(
          "version " + version + " or later of plugin '" + pluginShortName
              + "' needs to be installed",
          failIfMissing || failOnMissingPlugin
      );
    } else if (plugin.getWrapper().getVersionNumber().isOlderThan(new VersionNumber(version))) {
      failOrMarkBuildAsUnstable(
          "plugin '" + pluginShortName + "' needs to be updated to version " + version
              + " or later",
          failIfMissing || failOnMissingPlugin
      );
    }
  ...
  public void requireMinimumCoreVersion(String version) {
    if (!isMinimumCoreVersion(version)) {
      failOrMarkBuildAsUnstable("Jenkins needs to be updated to version " + version + " or later",
          false);
    }
  }
  ...
  public boolean isMinimumPluginVersionInstalled(String pluginShortName, String version) {
    Plugin plugin = Jenkins.get().getPlugin(pluginShortName);
    return plugin != null && !plugin.getWrapper().getVersionNumber()
        .isOlderThan(new VersionNumber(version));
  }
  ...
  public Integer getVSphereCloudHash(String name) {
    ...
    if (jenkins.getPlugin("vsphere-cloud") != null) {
      for (Cloud cloud : jenkins.clouds) {
        if (cloud instanceof vSphereCloud && ((vSphereCloud) cloud).getVsDescription()
            .equals(name)) {
  ...
  public void renameJobMatching(final String previousNames, String destination) throws IOException {
    ...
    Collection<Job> matchingJobs = Collections2.filter(items,
        topLevelItem -> topLevelItem.getRelativeNameFrom(context).matches(previousNames));
  ...
  public Node callExtension(String name, javaposse.jobdsl.dsl.Item item,
    ...
    Set<DslExtension> candidates = ExtensionPointHelper
        .findExtensionPoints(name, contextType, args);
    if (candidates.isEmpty()) {
      LOGGER.fine(
          "Found no extension which provides method " + name + " with arguments " + Arrays
              .toString(args)
      );
      return null;
  ...
  private boolean viewTypeChanged(View view, InputStream config) throws IOException {
    Class viewType = Jenkins.XSTREAM2.getMapper()
  ...
  private void createNewItem(String path, javaposse.jobdsl.dsl.Item dslItem) {
    ...
    try {
      ...
      if (parent instanceof ModifiableTopLevelItemGroup) {
        ...
        throw new DslException(format(Messages.CreateItem_UnsupportedParent(), parent.getFullName(),
            parent.getClass()));
      }
    } catch (IOException e) {
      throw new DslException(e);
  ...
  private void renameJob(Job from, String to) throws IOException {
    ...
    if (toParent == null) {
      throw new DslException(
          format(Messages.RenameJobMatching_UnknownParent(), from.getFullName(), to));
    }
  ...
  private static <I extends AbstractItem & TopLevelItem> void move(Item item,
      DirectlyModifiableTopLevelItemGroup destination) throws IOException {
